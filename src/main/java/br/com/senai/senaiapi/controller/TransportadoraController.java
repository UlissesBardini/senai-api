...
public class TransportadoraController {
	...
	@PostMapping
	public ResponseEntity<?> inserir(Map<String, Object> transportadoraMap) {
		Transportadora novaTransportadora = mapper.convertValue(transportadoraMap, Transportadora.class);
		Transportadora transportadoraSalva = service.inserir(novaTransportadora);
		return ResponseEntity.created(URI.create("/transportadoras/id/" + transportadoraSalva.getId())).build();
	}
	
	public ResponseEntity<?> alterar(@RequestBody Map<String, Object> transportadoraMap) {
		Transportadora transportadoraSalva = mapper.convertValue(transportadoraMap, Transportadora.class);
		Transportadora transportadoraAtualizada = service.alterar(transportadoraSalva);
		return ResponseEntity.ok(mapConverter.toJsonMap(transportadoraAtualizada));
	}
