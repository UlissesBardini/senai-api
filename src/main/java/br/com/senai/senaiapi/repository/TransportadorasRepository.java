...
public interface TransportadorasRepository extends JpaRepository<Transportadora, Integer> {
	...
	@Query(value = "SELECT t FROM Transportadora t WHERE Upper(t.nomeFantasia) LIKE Upper(:nome)")
	public List<Transportadora> listarPor(@Param("nome") String nomeFantasia);
	
	@Query(value = "SELECT t FROM Transportadora t WHERE t.id = :id")
