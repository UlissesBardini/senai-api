...
public class MotoristaService {	
	...
	public Motorista inserir(@Valid @NotNull Motorista novoMotorista) {
		Preconditions.checkArgument(novoMotorista.getId() == null, "O id do motorista deve ser nulo!");
		Motorista motoristaSalvo = repository.save(novoMotorista);
		return motoristaSalvo;
	}
	
	public Motorista alterar(@Valid @NotNull(message = "O motorista é obrigatório!") Motorista motoristaSalvo) {
		Preconditions.checkArgument(motoristaSalvo.getId() != null, "O id do motorista não pode ser nulo!");
		Motorista motoristaAtualizado = repository.save(motoristaSalvo);
		return motoristaAtualizado;
	}
	
	public void excluirPor(@NotNull(message = "O id para exclusão não pode ser nulo!") Integer id) {
